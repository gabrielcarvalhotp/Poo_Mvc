...
uses
  CRUD.Interfaces.Conexao, FireDAC.Comp.Client, Data.DB, FireDAC.DApt,
  FireDAC.Stan.Def;
...
type
  ...
  public
    ...
    function locate(aColunName, aValue : string) : iQuery;
  end;
...
function TQuery.FieldByName(aValue : string) : variant;
begin
  result := FQuery.FieldByName(aValue).Value;
end;

function TQuery.locate(aColunName, aValue : string) : iQuery;
begin
  result := self;
  FQuery.Locate(aColunName, aValue, [loPartialKey]);
end;
