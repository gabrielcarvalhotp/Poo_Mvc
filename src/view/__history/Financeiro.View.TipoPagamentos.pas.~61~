...
begin
  ...
  edId.Text := FTipoPgto.FDQuery.FieldByName('ID');
  edNome.Text := FTipoPgto.FDQuery.FieldByName('NOME');
end;
...
begin
  ...
  then
  begin
    id := FTipoPgto.FDQuery.FieldByName('ID');
    TControllerTipoPgto.DAO.DeleteTipoPagamentos(id);

    CardPanelPrincipal.ActiveCard := CardPesquisa;
    FTipoPgto.Pesquisa.Todos('TIPO_PAGAMANTOS').DataSet(DSTipoPgto);
    ClearComponents;
  end;
end;
...
begin
  ...
  begin
    ...
    if FStatus = TUtilitarios.Insert then
      TControllerTipoPgto.DAO.CreateTipoPagamentos(FTipoPgto)
    else if FStatus = TUtilitarios.Edit then
    begin
      FTipoPgto.IdNome.id(edId.Text);
      TControllerTipoPgto.DAO.EditTipoPagamentos(FTipoPgto);
    end;
